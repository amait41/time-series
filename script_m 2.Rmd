...
```{r}
timeSeries = ts(data$n_orders[1:308], frequency=7)
plot(timeSeries, type='l')
```
La série n'est clairement pas stationnaire. On peut dans un premier temps tenter de réduire la variance par une transformation de BoxCox (avec lambda = -0.7)
...
trainTime = 1:280
testTime = 281:308
mod = auto.arima(timeSeries[trainTime])
summary(mod)
```
