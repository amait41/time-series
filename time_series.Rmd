...
```{r}
decompose = decompose(trainset)
checkupRes(decompose$random[which(!is.na(decomp$random))])
Box.test(decompose$random)
kpss.test(decompose$random)
#adf.test(decompose$random[which(!is.na(decomp$random))])
adf.test(decompose$random)
```
