...
```{r, echo=FALSE}
acf(as.numeric(BCTS1), lag.max=40)
pacf(as.numeric(BCTS1), lag.max=40)
```
En effet, un motif périodique se dessine clairement. On voit néanmoins sur le graphe qu'on a réussi à réduire la tendance. Pour éliminer le motif périodique, essayons une différenciation d'ordre 7


## Différenciation I-B⁷

```{r, echo=FALSE}
BCTS2 = diff(bc_trainset,7)
plot(BCTS2)
```
...
```{r, echo=FALSE}
model3 = auto.arima(bc_trainset, allowmean=TRUE, D=1, d=0, allowdrift=TRUE)
model3
```
Les termes ar2, ar3 et ar4 ne sont pas significatifs, on va donc modifier un peu notre modèle.

```{r, echo=FALSE}
model3 = auto.arima(bc_trainset, allowmean=TRUE, max.p = 1, max.q = 1, D=1, d=0, allowdrift=TRUE)
model3
```
