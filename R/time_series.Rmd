...
```
Les modèles $ARMA$ font l'hypothèse que la série temporelle $(X_t)$ est stationnaire (du second ordre), c'est à dire que sa moyenne et sa covariance sont invariantes par translation dans le temps:<br>
- $\mathbb{E}[X_t]=m(t)=m$ pour tout $t \in \mathbb{Z}$.<br>
- $Cov(X_t, X_s) = \Gamma(t, s) = \Gamma(t+k, s+k)$ pour tout $(t,s) \in \mathbb{Z}^2$ et tout décalage temporelle $k \in \mathbb{Z}$.<br>

Cette hypothèse est nécéssaire car ... .

En plus de l'aspect visuel, nous utiliserons les tests suivants pour étudier sur la stationarité :<br>
- Le test ADF avec $\mathcal{H}_0$ : "la trajectoire est issue d'un processus non stationnaire" contre $\mathcal{H}_1=\bar{\mathcal{H}_0}$.<br>
- Le test KPSS avec $\mathcal{H}_0$ : "la trajectoire est issue d'un processus stationnaire" contre $\mathcal{H}_1=\bar{\mathcal{H}_0}$.<br>
...
=> Remarque sur la stat visuellement, transformation
(Cependant on voit bien que la variance ne reste pas constante, ce que les tests ADF et KPSS ne détectent pas.)
...
Nous présentons maintenant deux outils indispensable à l'étude des séries temporelles: l'ACF et la PAFC.

L'AFC représente les autocorrélation entre deux valeurs distantes de $h$ dans le temps. Une autocorrélation nulle à partir d'un rang $q+1$ est caractéristique d'un processus $MA(q)$. L'autocorrélogramme PAFC représente quand à lui les corrélation "pur" entre deux valeurs distantes de $h$ dans le temps, c'est à dire entre lesquelles on a supprimé l'influence linéaire des valeurs intermédiaires. Une PACF avec des pics dans le couloir de non-significativité à partir du rang $p+1$ est caractéristique d'un processus $AR(p)$.

```{r fig.width = 15, fig.height=5, echo=FALSE}
layout(matrix(c(1, 2), nrow=1, ncol=2, byrow=TRUE))
acf(trainset, lag.max = 50, main="")
pacf(trainset, lag.max = 50, main="")
```
...
En conclusion, la série n'est pas stationnaire et présente une saisonalité hebdomadaire ainsi qu'une variance fluctuante au fil du temps.

Pour rendre cette série stationnaire, nous allons utiliser plusieurs approches tel que la décomposition (partie 2.1) et la différentiation (partie 2.2). Puis nous construirons et comparons nos modèle $ARIMA$ à l'aide des fonction `auto.arima` et `checkup_res` (partie 3) avant d'évaluer nos modèles sur le jeu de données test avec des critères prédictifs  (partie 4).
...
```
Visuellement, les regression d'ordre 3 et 4 donnent de très bon résultat et obtienent un R² ajusté de 0.88. Le principe de parcimonie nous pousse à garder la régression linéaire d'ordre 3.
