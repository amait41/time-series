...
pred_decomp = function(trainset, testset){
  ...
  x2 = x**2
  x3 = x**3
  trend = reg_lin$coefficients[1] + x * reg_lin$coefficients[2] + x2 * reg_lin$coefficients[3] + x3 * reg_lin$coefficients[4]
  if (n2%%7==0){
    print("ok")
    seasons = c(rep(decomp$figure, n2%/%7))
  }else{
    print(n2)
    print(n2%/%7)
    seasons = c(rep(decomp$figure, n2%/%7), decomp$figure[1:n2%%7])
  }
  trend + seasons
}

plot(pred_decomp(trainset, testset), type="l", ylab="Prédiction")
```
...
```{r}
diff_decompose = diff(diff(decomp$random, 7))
...
```{r}
model0 = auto.arima(decomp$random, allowmean=TRUE, allowdrift = TRUE)
model0
```
...
pred0 = forecast(model0, h=n)$mean + pred_decomp(trainset, testset)

models = c("decomp + SARIMA",
           "SARIMA (d=0, D=1)",
           "SARIMA (d=1, D=1)",
           "log + SARIMA (d=1, D=1)",
           "log + SARIMA (d=1, D=1)",
           "BoxCox + SARIMA (d=1, D=1)",
           "Box-CoxSARIMA (d=1, D=1)",
           "testset")
...
plot(ts(testset, frequency=7, start=c(41,1)), ylim=c(18, 65))
...
lines(ts(pred6$mean, frequency=7, start=c(41,1)), col="blue")
legend("topleft", legend=models, col=c("gray", "blue", "red", "purple", "orange", "brown", "blue", "black"), lw=1, box.lty=0, cex = 0.6)
```
...
perf = rbind(perf0, perf1, perf2, perf3, perf4, perf5, perf6)
