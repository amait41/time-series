...
plot(trainset, type='l', xlab="Période", ylab="# commades")
```
...
lm2 = lm(X2~t2)
lm3 = lm(X3~t3)
plot(t1, X1, type='l', xlab="Index", ylab="# commandes", xlim=c(0,length(trainset)))
...
```
Poursuivons l'analyse de la série avec l'étude de l'AFC, de la PAFC ainsi que des tests de stationarité.
...
En conclusion, la série n'est pas stationnaire car elle présente une saisonalité hebdomadaire et une variance qui évolue au fil du temps. Il peut être intéressant d'appliquer des transformations pour atténuer les variations de cette dernière.
...
```{r}
decomp = decomp(trainset, testset)
plot(decomp$train)
...
```
```{r}
model1 = auto.arima(trainset, allowmean=TRUE, allowdrift = TRUE)
model1
```
