...
Les coefficients sar et sma ne sont pas significatifs, on peut tenter de rétrograder notre modèle

```{r}
mod = Arima(BoxCoxTS, order = c(1,0,1), seasonal = list(order=c(0,1,0), period=7))
mod
```
Néanmoins on voit que celui-ci est moins bon au sens de l'AIC et du BIC, on va donc plutôt garder le modèle complet
...
pred_val = boxcox.inv(pred$mean,-0.7)
